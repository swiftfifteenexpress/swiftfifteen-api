...
export default async function handler(req, res) {
  ...
  if (missing.length) {
    return res.status(400).json({
      ...
      optional_fields: [
        'sender_email', 'pickup_state', 'pickup_city', 'delivery_state', 'delivery_city',
        'package_description', 'package_weight', 'declared_value', 'pickup_date', 'partner_order_id'
      ]
    });
  }
  ...
  // Qualify with city/state (if the partner sends them) so geocoding doesn't
  // guess wrong when the same street name exists in multiple states
  ...
  try {
    ...
    const [senderCode, receiverCode] = await Promise.all([
      validateAddress(
        pickupFullAddress, sender_name, sender_phone,
        sender_email || 'orders@swiftfifteenexpress.com',
        SB_KEY, pickupCoords
      ),
      validateAddress(
        deliveryFullAddress, recipient_name, recipient_phone,
